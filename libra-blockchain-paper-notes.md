...
2) Run prologue (gas metering disabled):
- It's a procedure of the Move LibraAccount module

  2.1) Check hash of sender's PK == auth key stored in sender's account
  2.2) Check gas price * max gas amount <= sender's account balance
  2.3) tx's sequence number == sequence number in sender's account
